...
if(isset($_POST["submit"])) {
  if(tambahPeserta($_POST)>0) {
    echo "<script>alert('Data Peserta Berhasil Ditambah');document.location.href = 'peserta_show.php';</script>";
  } else {
    echo "<script>alert('Data Peserta Gagal Ditambah');document.location.href = 'peserta_show.php';</script>";
  }
}
...
<html lang="en">
  <head>
    ...
    <title>Admin - Tambah Data Peserta</title>
  ...
  <body>
    ...
    <section id="body">
      <div class="container">
        <h1 class="text-primary mb-5 text-center fw-bold">Tambah Data Peserta <br> Pelatihan Digital Marketing</h1>
        <div class="row mb-3">
          <div class="col-md-9">
        ...
        <div class="row">
          ...
          <form method="post">
            <div class="mb-3">
              ...
              <select class="form-select" name="kode_partisipan" id="kode_partisipan" autocomplete="off" required>
                <option selected></option>
                <?php 
                  $sql_kodepartisipan = mysqli_query($conn, "SELECT * FROM tb_pendaftar ORDER BY kode_partisipan ASC") or die (mysqli_error($conn));
                  while($data_kodepartisipan = mysqli_fetch_array($sql_kodepartisipan)) {
                    echo '
                    <option value="'.$data_kodepartisipan['kode_partisipan'].'">
                      '.$data_kodepartisipan['kode_partisipan'].' - '.$data_kodepartisipan['nama_pendaftar'].'
                    </option>
                    ';
                  }
                ?>
            ...
            <div class="mb-3">
              ...
              <select class="form-select" name="nama_kelas" id="nama_kelas" required>
                <option selected></option>
                <?php 
                ...
                while($data_namakelas = mysqli_fetch_array($sql_namakelas)) {
                  ...
                  <option value="'.$data_namakelas['nama_kelas'].'">
                    '.$data_namakelas['nama_kelas'].' - '.$data_namakelas['nama_pengajar'].'
                  </option>
                  ';
                }
              ?>
            ...
            <button type="submit" name="submit" class="btn btn-primary"><i class="bi bi-plus-circle-fill"></i> Tambah Data Peserta</button>
