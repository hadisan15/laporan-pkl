...
$tb_peserta = queryTampilPeserta("SELECT * FROM tbjoin_pesertapendaftar WHERE id_peserta = $id_peserta")[0];

if(isset($_POST["submit"])) {
  if(ubahPeserta($_POST)>0) {
    echo "<script>alert('Data Peserta Berhasil Diubah');document.location.href = 'peserta_show.php';</script>";
    // echo mysqli_error($conn);
  } else {
    echo "<script>alert('Data Peserta Gagal Diubah');document.location.href = 'peserta_show.php';</script>";
  }
}
...
<html lang="en">
  ...
  <body>
    ...
    <section id="body">
      <div class="container">
        <h1 class="text-primary mb-5 text-center fw-bold">Ubah Data Peserta <br> Pelatihan Digital Marketing</h1>
        <div class="row mb-3">
          <div class="col-md-9">
            <a href="peserta_show.php">
              <button type="button" class="btn btn-outline-primary"><i class="bi bi-table"></i> Tabel Data Peserta</button>
            </a>
          </div>
        </div>
        <div class="row">
          <div class="col">
          <form method="post">
            <input type="hidden" name="id" value="<?= $tb_peserta["id_peserta"]; ?>">
            <div class="mb-3">
              <label for="kode_partisipan" class="form-label">Kode Partisipan</label>
              <input type="text" class="form-control" id="kode_partisipan" name="kode_partisipan" value="<?= $tb_peserta["kode_partisipan"] ?>" required autocomplete="off" disabled>
            </div>
            <div class="mb-3">
                <label for="nama_pendaftar" class="form-label">Nama Peserta</label>
                <input type="text" class="form-control" id="nama_pendaftar" name="nama_pendaftar" value="<?= $tb_peserta["nama_pendaftar"] ?>" required autocomplete="off" disabled>
              </div>
            <div class="mb-3">
              <label for="nama_kelas" class="form-label">Nama Kelas</label>
              <select class="form-select" name="nama_kelas" id="nama_kelas">
                <option selected><?= $tb_peserta["nama_kelas"] ?></option>
                <?php 
                $sql_namakelas = mysqli_query($conn, "SELECT * FROM tb_kelas ORDER BY nama_kelas ASC") or die (mysqli_error($conn));
                while($data_namakelas = mysqli_fetch_array($sql_namakelas)) {
                  echo '
                  <option value="'.$data_namakelas['nama_kelas'].'">
                    '.$data_namakelas['nama_kelas'].'
                  </option>
                  ';
                }
              ?>
